...
public class StagePerder extends Stage {
    ...
    StagePerder(Viewport viewport, Batch batch, StarBlast menu) {
        ...
        this.menu = menu;
        Image background = new Image(Constants.MANAGER.get("PantallaPerder/FondoDerribado.jpg", Texture.class));
        background.setPosition(Constants.SCREEN_WIDTH / 2 - background.getWidth() / 2,
                Constants.SCREEN_HEIGTH / 2 - background.getHeight() / 2);
        addActor(background);
        ...
        countdownAnimation = new AnimatedImage(new Animation<TextureRegion>(1f, new TextureRegion(Constants.MANAGER.get("PantallaPerder/Countdown.png", Texture.class)).split(281, 280)[0]));
